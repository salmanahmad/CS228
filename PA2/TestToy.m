...
toy_clique_tree = CreateCliqueTree(toy_network.names, toy_network.dim, toy_factors);
toy_cluster_graph = CreateClusterGraph(toy_network.names, toy_network.dim, toy_factors);
toy_evidence = zeros(1, length(toy_network.names));

% Exact Inference
ExactM = ExactInference(toy_clique_tree, toy_factors, toy_evidence);
...
for j = 1:length(transition_names)
    ...
    vis_vars = [3];
    VisualizeMCMCMarginals(samples_list, vis_vars, toy_network.dim(vis_vars), toy_factors, ...
                           500, ExactM(vis_vars));
